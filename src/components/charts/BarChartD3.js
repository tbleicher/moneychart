...
function createChart(dom, props) {
  ...
  function drawBarChart(data, range) {
    ...
    var barGap = 10;
    var barWidth =
      (width - (margin.left + margin.right)) / days.length - barGap;
    ...
    var barGroup = chart.selectAll(".bar").data(data);
    ...
    barGroup
      ...
      .on("mouseover", function(d, i) {
        ...
        tooltip.select("rect").style("fill", d.color);
  ...
  function getBalanceDomain(data, padding = 0.0) {
    var higher = d3.max(
      data.map(function(d) {
        return d.balance + d.debit;
      })
    );
    var lower = d3.min(
      data.map(function(d) {
        return d.balance - d.credit;
      })
    );
  ...
  var xScaleBrush = d3.scaleTime().range([0, chartWidth]);
  ...
  var yContext = d3.scaleLinear().range([contextHeight, 0]);

  var yAxis = d3.axisLeft().scale(yScale);

  var yAxisContext = d3.axisLeft().scale(yContext);
  ...
  var areaContext = d3
    ...
    .x(function(d) {
      return xScaleBrush(d.valueDate);
    })
    .y0(contextHeight)
    .y1(function(d) {
      return yContext(d.balance);
    });
